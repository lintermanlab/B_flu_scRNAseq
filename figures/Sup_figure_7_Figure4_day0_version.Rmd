...
knitr::opts_chunk$set(echo = F, tidy = T, warning = F, message = F, fig.width = 9, fig.height = 10, tidy.opts=list(width.cutoff=60))
```
...
ages <- levels(ALFNA16$Age.Group)

ages <-ifelse(ages == "young", "22-36yo",
              ifelse(ages == "young", "22-36yo","67-86yo"))
...
cowplot::plot_grid(
  ...
  cowplot::plot_grid( # First 4 panels:
            ...
            ggstripchart(data = muts,
             x= "age", y= "num_mutations",# ylim = ylim,
             ylab = "\n# of IgH mutations",
             ylim = c(0,750),
             facet.by = c("day"), add = "violin", add.params = list(color = "grey", fill = NA),
  ...
  stat_compare_means(method = "wilcox", 
                     ...
                     label.x.npc = "left") + 
  theme_pubr(legend = "top", base_size=10) + 
  theme( legend.title = element_blank()),

                      cowplot::plot_grid(
        # 
       ggstripchart(data = muts, x = "age", y = "num_fr_mutations",facet.by = "day",
          xlab = "age", ylab = "\n# mutations [FR]",
          ylim = c(0,500),
          shape = 21,
          fill = "age",
  ...
  ggstripchart(data = muts, x = "age", y = "num_cdr_mutations",facet.by = "day",
          xlab = "age", ylab = "\n# mutations [CDR]",
          ylim = c(0,500),
          shape = 21,
          fill = "age",
...
cowplot::plot_grid(
  # R/S 
  ggstripchart(data = muts, x = "age", y = "fr.RS.ratio", facet.by = "day",
            xlab = "age", ylab = "\nPseudolog R/S ratio [FR]",
            ylim = c(0, 50000),
            shape = 21,
            fill = "age",
            palette = c("white", "darkgrey")) + 
  ...
  ggstripchart(data = muts, x = "age", y = "cdr.RS.ratio", facet.by = "day",
               xlab = "age", ylab = "\nPseudolog R/S ratio [CDR]",
               ylim = c(0, 50000),
               shape = 21,
               fill = "age",
               palette = c("white", "darkgrey")) + 
  ...
  cowplot::plot_grid( # want both of these to be a single column: 
   #### Number of IgH mutations by UMAP clusters and day / age FRAMEWORK ####
                     ggstripchart(data = muts,
                       x = "age", y = "num_fr_mutations", facet.by = c("day","clusters"), 
                       col = "clusters",
            palette = tableau10medium[1:5],
            ylim = c(0,500),
          xlab = "", ylab = "FR\n# mutations",
            legend = "none",
          panel.labs = list(clusters =c("*FCRL5*+<br>B mem","*MALAT1*+<br>B mem","*LY9*+<br>B mem","naive","*MARCKS*+<br>B mem"))) + 
            ...
            theme_pubr(legend = "none", base_size=10) + 
            rotate_x_text() + 
            theme(axis.text.x = element_blank(), strip.background = element_rect(fill = NA, colour=NA),
                  strip.text = ggtext::element_markdown()),
 
       ggstripchart(data = muts,
                       x = "age", y = "fr.RS.ratio", facet.by = c("day","clusters"), 
                       col = "clusters",
            palette = tableau10medium[1:5],
          xlab = "", ylab = "FR\nPseudolog R/S ratio",
          ylim = c(0, 50000),
  ...
  cowplot::plot_grid( # want both of these to be a single column: 
      #### Number of IgH mutations by UMAP clusters and day / age CDR ####
                     ggstripchart(data = muts,
                       x = "age", y = "num_cdr_mutations", facet.by = c("day","clusters"), 
                       col = "clusters",
            palette = tableau10medium[1:5],
          xlab = "", ylab = "CDR\n# mutations",
          ...
          panel.labs = list(clusters =c("*FCRL5*+<br>B mem","*MALAT1*+<br>B mem","*LY9*+<br>B mem","naive","*MARCKS*+<br>B mem"))) + 
            ...
            theme(axis.text.x = element_blank(), 
                  ...
                  strip.text = ggtext::element_markdown()),
                    ...
                    ggstripchart(data = muts,
                       x = "age", y = "cdr.RS.ratio", facet.by = c("day","clusters"), 
                       col = "clusters",
            palette = tableau10medium[1:5],
          xlab = "", ylab = "CDR\nPseudolog R/S ratio",
          ylim = c(0, 50000),
...
(B) and (C) The number of nucleotide mutations within the antibody heavy chain are shown for each region at day 0 and for older and younger individuals for FR (framework regions, B) or CDR (complementarity determining regions, C).
...
(E) and (F) The ratio of replacement:silent mutations within the heavy chain is shown for each cell from day 0 from both age groups.
(G) and (H) The number of mutations in the antibody heavy chain at day 0 plotted by age group for each UMAP cluster, for FR (G) or CDR (H).
(I) and (J) The ratio of replacement:silent mutations the antibody heavy chain at day 0 plotted by age group for each UMAP cluster, for FR (I) or CDR (J).
