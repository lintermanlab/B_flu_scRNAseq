...
##
Vfam <- table(paste(sce[,sce$day == "d42" ]$clusters, sce[,sce$day == "d42" ]$age,  sce[,sce$day == "d42" ]$PID), 
              substr(sce[,sce$day == "d42" ]$v_call,1,5))
...
# The number of NAs:
Vfam$`no call` <- table(paste(sce[,sce$day == "d42" ]$clusters, sce[,sce$day == "d42" ]$age,  sce[,sce$day == "d42" ]$PID)) - rowSums(Vfam)
#Vfam
...
Vfam.prop[,2:9] <- apply(Vfam.prop[,2:9], 2, function(x) { x / rowSums(Vfam.prop[,2:9])})
...
Vfam.clusters.prop[,3:10] <- apply(Vfam.clusters.prop[,3:10], 2, function(x) { x / rowSums(Vfam.clusters.prop[,3:10])})
...
cowplot::plot_grid(
  cowplot::plot_grid(
  plotUMAP(sce[, sce$productive %in% c("T")], colour_by="Isotype.Constant.group") +
    scale_fill_brewer(palette = "Set1") + 
    theme_pubr() +
    guides(fill = guide_legend(
      title = "Secreted Ig\nclass", title.position = "top", nrow = 3)) +
    ...
    # QUANTITATION OF UMAP:
    ggbarplot(data = colData(sce[, sce$productive %in% c("T")]) %>%
            as.data.frame() %>%
            group_by(named_clusters, Isotype.Constant.group) %>%
            dplyr::summarise(n = n()) %>%
            mutate(freq = n / sum(n)), 
          
          fill = "Isotype.Constant.group", 
          y = "freq", 
          ylab = "Proportion",
          xlab = "",
          x = "named_clusters", alpha = 0.6) + 
  scale_fill_brewer(palette = "Set1") +
  ...
  # IGH V family usage PROPORTIONS at d42, faceted by UMAP cluster:
...
#####
  ...
  cowplot::plot_grid(
    ...
    ## plot_grid for third row: 3 columns, the last column itself split in 3 rows.
    
  # IGH V segment usage at d42:
  ggbarplot(data = reshape2::melt(table(gsub(sce[,sce$day == "d42" ]$igh, pattern = "_.*$", replacement = ""), sce[,sce$day == "d42"]$age)) %>%
  ...
  # HM of characteristics of bnAbs:
  HM$gtable,
  
  # Column of plots:
  cowplot::plot_grid(
    ggpaired(data = sum.dat.wide.bnAb.paired %>% .[order(.$PID),] %>% .[order(.$day), ], 
             x= "day", y = "pct",
             facet.by = "age",
             xlab = "",
             ylab = "bnAb BCRs\nProportion of cells",
      ...
      stat_compare_means(paired = T, 
                         aes(label = paste0( ..p.format..)), label.y = 0.35) +
        theme(strip.background = element_rect(fill = NA)), 
    
    # ggboxplot(data = res, x = "age", y = "ratio", xlab = "", ylab = "bnAb\nd42/d0", add = "jitter", outlier.shape = NA, ylim = c(0, 4)) + stat_compare_means(aes(label = paste0("p=", ..p.format..)), label.y = 3.75) + geom_hline(yintercept = 1, linetype = 2, col = "grey"),
    
ggbarplot(data = abundances %>%
            dplyr::filter(bnAb == "bnAb") %>%
            dplyr::group_by(age, day, clusters, .drop = FALSE) %>% # the drop = FALSE switch keeps empty rows (here clusters with no bnAb/IGHV1-69+ cells)
            dplyr::summarise(num_of_cells = n()) %>%
            dplyr::group_by(age, day) %>%
            dplyr::mutate(pct_of_cells = num_of_cells / sum(num_of_cells)),
          x = c("day"), y = "pct_of_cells",
          facet.by = "age", 
          fill = "clusters", pal = tableau10medium,
          ylab ="Proportion of\n 1-69 bnAb+ cells", xlab = "") + 
  theme_pubr(legend = "bottom")  + 
  scale_y_continuous(breaks = c(0, 1)) + 
  scale_x_discrete(labels = c(0, 42)) +
...
__Figure 3 – Differences in heavy chain V segment usage, including broadly neutralizing *IGHV1-69*, in aged individuals after TIV immunization__

(A) UMAP embedding as previously, showing secreted immunoglobulin classes. Isotypes are combined to immunoglobulin class (eg. IgG1, IgG2, IgG3, IgG4 grouped as IGHG): `r paste(fig.legend, collapse = "; ")`.
(B) For each UMAP cluster, the proportion of secreted immunoglobulin classes are plotted. Each immunoglobulin class is indicated with the same shading as in (A).
(C) V segment family usage in the immunoglobulin heavy chain at day 42 for younger and older individuals. *P*=`r signif(fisher.test(table(substr(sce[,sce$day == "d42" ]$v_call,1,5), sce[,sce$day == "d42"]$age), simulate.p.value = T)$p, digits = 2)`, by Fisher's test.
(D) V segment family usage by HA-specific immunoglobulin heavy chains at day 42 for younger and older individuals expressed as the proportion of the cells separated by each UMAP cluster.
(E) V allele usage in the immunoglobulin heavy chain at day 42 for younger and older individuals. *P*=`r signif(fisher.test(table(gsub(sce[,sce$day == "d42" ]$igh, pattern = "_.*$", replacement = ""), sce[,sce$day == "d42"]$age), simulate.p.value = T)$p, digits = 2)`, by Fisher's test.
(F) Heatmap summarising the presence of biophysical attributes characteristic of broadly neutralizing antibody for *IGHV1-69* B cells. The presence of F54, a hydrophobic residue at 53 and a tyrosine at 97, 98 or 99 are shown. An antibody is expected to be a bnAb if F54, there is a hydrophobic residue at position 53 and a Y within the CDR3. Presence of a characteristic is shown by a green box and empty boxes reflect its absence. On the left hand side of the panel, the age group of the cell is shown (cells from 18-36 year olds and 65-98 year olds in white and grey respectively) and the day of the sample is shown (day 0 and day 42 in white and black respectively). The individual identifiers are listed next to each row and some individuals.
(G) The proportion of B cells from each study day that encode *IGHV1-69* bnAbs. Individual B cells were filtered based on productive heavy chains, and only those individuals with paired data from day 0 and day 42 are shown (requires > 1 succesfully filtered B cell at both days). BnAbs were defined as shown in (F). Grey lines link the same individual. Paired 2 tailed Mann-Whitney *P* values are shown.
(H) The proportion of *IGHV1-69* B cells within each UMAP cluster for both age groups and on days 0 and 42 after TIV.
