...
cowplot::plot_grid( # second chunk of figure. Need 2 columns:
  ...
  cowplot::plot_grid( # want both of these to be a single column: 
   #### Number of IgH mutations by UMAP clusters and day / age FRAMEWORK ####
                     ggstripchart(data = muts,
                       x = "clusters", y = "num_fr_mutations", facet.by = c("day"), 
                       col = "clusters",
          ...
          panel.labs = list(clusters =c("*FCRL5*+<br>B mem","*MALAT1*+<br>B mem","*LY9*+<br>B mem","naive","*MARCKS*+<br>B mem"))) + 
            ...
            stat_compare_means(aes(label = paste0( ..p.adj..)), method ="wilcox",comparisons = list(c(1,2), c(1,3), c(1,4), c(1,5))) + 
     scale_y_continuous() +
       ...
       ggstripchart(data = muts,
                       x = "clusters", y = "fr.RS.ratio", facet.by = c("day"), 
                       col = "clusters",
            palette = tableau10medium[1:5],
          xlab = "", ylab = "FR\nPseudolog R/S ratio",
          ylim = c(0, 1000000),
          ...
          panel.labs = list(clusters =c("*FCRL5*+<br>B mem","*MALAT1*+<br>B mem","*LY9*+<br>B mem","naive","*MARCKS*+<br>B mem"))) + 
            geom_boxplot(outlier.shape = NA, fill = NA) + 
            stat_compare_means(aes(label = paste0( ..p.adj..)), comparisons = list(c(1,2), c(1,3), c(1,4), c(1,5))) +  
            scale_y_continuous(trans = scales::pseudo_log_trans(), breaks = c(0, 10, 100, 1000, 10000, 100000)) +
            theme_pubr(legend = "none") + 
            rotate_x_text() + 
            theme(strip.background = element_rect(fill = NA),
                  strip.text = ggtext::element_markdown()),
  labels = c("A", "B"),
  rel_heights = c(1,1), # give the last plot a bit more height as it has age group labels
  align = "v",
  ncol = 1),
  ...
  cowplot::plot_grid( # want both of these to be a single column: 
      #### Number of IgH mutations by UMAP clusters and day / age CDR ####
                     ggstripchart(data = muts,
                       x = "clusters", y = "num_cdr_mutations", facet.by = c("day"), 
                       col = "clusters",
            palette = tableau10medium[1:5],
          xlab = "", ylab = "CDR\n# mutations",
          ...
          panel.labs = list(clusters =c("*FCRL5*+<br>B mem","*MALAT1*+<br>B mem","*LY9*+<br>B mem","naive","*MARCKS*+<br>B mem"))) + 
            ...
            theme(axis.text.x = element_blank(), 
                  ...
                  strip.text = ggtext::element_markdown()),
                    ...
                    ggstripchart(data = muts,
                       x = "clusters", y = "cdr.RS.ratio", facet.by = c("day"), 
                       col = "clusters",
            palette = tableau10medium[1:5],
          xlab = "", ylab = "CDR\nPseudolog R/S ratio",
          ylim = c(0, 1000000),
...
__Supplementary Figure 8 – Somatic hypermutation is reduced in haemagglutinin specific memory B cells from aged individuals after TIV immunisation__
